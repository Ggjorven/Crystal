...
void Tile::OnRender()
{
	Renderer2D::DrawQuad({ m_Position.x * s_GlobalScale, m_Position.y * -1.0f * s_GlobalScale }, { m_Size.x * s_GlobalScale, m_Size.y * s_GlobalScale }, { m_Size.x / 2.0f * s_GlobalScale, -(m_Size.y / 2.0f * s_GlobalScale) }, m_Texture, m_TexCoords, false, Layer2D::GetSceneCamera()->GetCamera());
}
