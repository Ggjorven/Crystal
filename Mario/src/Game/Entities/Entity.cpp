...
{
	Renderer2D::DrawQuad({ m_Position.x, m_Position.y * -1.0f }, { m_Size.x * s_GlobalScale, m_Size.y * s_GlobalScale }, { m_Size.x / 2.0f * s_GlobalScale, -(m_Size.y / 2.0f * s_GlobalScale) }, m_Texture, m_TexCoords, m_Reversed, GameLayer::GetSceneCamera()->GetCamera());
}